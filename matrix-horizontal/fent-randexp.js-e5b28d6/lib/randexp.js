...
function toOtherCase(code) {
	return code + (code >= 97 && code <= 122 ? -32
...
function expand(token) {
	...
	for (let i = 0; i < token.set.length; i++) {
		const subrange = expand.call(this, token.set[i]);
		drange.add(subrange);
		if (this.ignoreCase) {
 ...
 * @param {String} m
 */
const RandExp = module.exports = function (regexp, m) {
...
function gen(token, groups) {
	let stack; let str; let n; let i; let
		l;
